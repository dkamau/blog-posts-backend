...
namespace BlogPostsBackend.Core.Services
{
    public class MailGunEmailService : IEmailService
    {
        ...
        public async Task SendEmailAsync(string to, string subject, string body)
        {
            try
            {
                string apiKey = Environment.GetEnvironmentVariable("BlogPosts_MAILGUN_API_KEY");
                string domain = Environment.GetEnvironmentVariable("BlogPosts_MAILGUN_DOMAIN");
                ...
                request.AddParameter("from", $"Blog Posts <info@{domain}>");
